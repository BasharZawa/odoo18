...
class BpmApiController(http.Controller):
    ...
    def start_process(self, key, **kwargs):
        env = request.env
        defn = env['bpm.process.definition'].sudo().search([('key','=', key), ('is_active','=', True)], order="version desc", limit=1)
        if not defn:
            return {'ok': False, 'error': 'Definition not found'}
        proc = env['bpm.process.instance'].sudo().create({
        ...
        request.env['bpm.activity.instance'].sudo().create({
    ...
    def push_event(self, **payload):
        ...
        subs = env['bpm.event.subscription'].sudo().search([('event_name','=', event_name),('correlation_key','=', correlation_key),('status','=','active')])
        for s in subs:
            request.env['bpm.orchestrator.helper'].sudo()._enqueue(s.proc_id, payload.get('next_node') or 'end', '')
    ...
    def complete_task(self, **kw):
        ...
        Act = request.env['bpm.activity.instance'].sudo(); act = Act.browse(act_id)
        if not act.exists():
        ...
        request.env['bpm.orchestrator.helper'].sudo()._on_branch_completed(proc, act)
        if nxt:
            request.env['bpm.orchestrator.helper'].sudo()._enqueue(proc, nxt, '')
        return request.redirect(f'/web#id={proc.id}&model=bpm.process.instance&view_type=form')
